...
class SGA2DR(nn.Module):

  def __init__(self, out_feat_drug_size, sga_size, embedding_dim, embedding_dim_last, can_size=10, n_head=5,
               num_attn_blocks=3,using_cancer_type=True,sga_gene_list=None,embed_pretrain_gene2vec=True):

    """ Initialize the model.
    hiddenLayerArch              A list defines number of hidden layers and number of hidden nodes per layer
    Shuangxia, please provide detailed annotation of input Parameters
    ----------
    """
    super(SGA2DR, self).__init__()
  ...
  def forward(self, ori_sga, sga_index, can_index):
    ...
    preds1 = self.hiddenLayers[-1](curr_hidden)
    return preds1, batch_attn_wts, hidden_layer_outs
