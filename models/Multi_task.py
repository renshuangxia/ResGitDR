...
class SGASelfAttn(nn.Module):
    ...
    def forward(self, sga, mask=True):
        ...
        if mask == True:
            ...
            # print("mask shape",sga_mask.shape)
            case_sga_attn_heads = case_sga_attn_heads.masked_fill(sga_mask ==0,-1e9)
            # print(case_sga_attn_heads)

        # normalize by softmax, leading to a n-by-m attention matrix
...
class Multitask(nn.Module):
  ...
  def __init__(self, out_feat_drug_size, out_feat_deg_size, sga_size, embedding_dim, embedding_dim_last, can_size=10, n_head=5,
    ...
    self.out_feat_drug_size = out_feat_drug_size #output drug dimension
    self.out_feat_deg_size = out_feat_deg_size #output deg dimension
    ...
    self.activationF = nn.ReLU()  # we can change different activation funciton
    ...
    # the second to last layer which has different dimension as before
    ...
    if using_tf_gene_matrix:
        self.hiddenLayers.append(nn.Linear(self.embedding_dim_last, self.out_feat_drug_size))
        self.layer_final_deg = nn.Linear(self.tf_gene.shape[0], self.out_feat_deg_size,bias=False)
        self.mask_value = self.tf_gene.T

        # define layer weight clapped by mask
        self.layer_final_deg.weight.data = self.layer_final_deg.weight.data * torch.FloatTensor(self.tf_gene.T)
        # register a backford hook with the mask value
        self.layer_final_deg.weight.register_hook(lambda grad: grad.mul_(self.mask_value))
        self.hiddenLayers.append(self.layer_final_deg)
    else:
        self.hiddenLayers.append(nn.Linear(self.embedding_dim_last, self.out_feat_drug_size))
        self.hiddenLayers.append(nn.Linear(self.embedding_dim_last, self.out_feat_deg_size))


  def forward(self, ori_sga, sga_index, can_index):
    """ Forward process.
      Parameters
      ----------
    ...
    for i in range(1, len(self.hiddenLayers)-1):
        ...
        curr_hidden = self.activationF(curr_hidden) if i < len(self.hiddenLayers) - 2 else torch.sigmoid(curr_hidden)
    ...
    preds1 = self.hiddenLayers[-2](curr_hidden)
    preds2 = self.hiddenLayers[-1](curr_hidden)

    return preds1, preds2, batch_attn_wts, hidden_layer_outs
