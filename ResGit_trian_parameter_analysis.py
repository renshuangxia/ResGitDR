...
'''
    This script is used to do the parameter analysis for ResGit model, so we used all data as training data 
    including TCGA and GDSC, no testing data.
'''
...
'''
    store result
'''
results_dir = "result/parameters/ResGit/"

def store_result_dir(results_dir):
    attn_dir = results_dir + 'attns/'
    emb_dir = results_dir + 'embs/'
    hidden_weights_dir = results_dir + 'hidden_weights/'
    res_dir = results_dir + 'results/'
    hidden_dir = results_dir + 'hidden/'
...
for epoch in range(num_epochs):
    ...
    if num_epoch_no_gain > early_stop or epoch == trim_starting_epoch:
        ...
        #trim embedding
            ...
            # if using using early stop for the first step, we will need to uncomment following two lines to use the best model
            ...
            for idx,sga_block in enumerate(model.SGA_blocks):
                ...
                if idx == num_blocks-1:
                    ...
                    common_gene_tf_sga = [common_gene for common_gene in embs_wt_df.index if common_gene in embs_wt_df.columns]
    ...
    if epoch_loss < min_loss:
        ...
        num_epoch_no_gain += 1
    scheduler.step(epoch_loss) #if loss doesn't reduce, try to lower the learning rate
...
for phase in ['train']:
    ...
    dirs_map = store_result_dir(results_dir)
    ...
    for idx, sga_block in enumerate(model.SGA_blocks):
        ...
        if idx == num_blocks-1:
            sga_embs_df = pd.DataFrame(sga_embs, index=sga_df.columns,columns=tf_gene_df.index)
        else:
    ...
    for idx, hidden_layer in enumerate(model.hiddenLayers):
        ...
        if idx == num_blocks-1:
            hidden_weights_df = pd.DataFrame(hidden_weights,index=deg_df.columns,columns=tf_gene_df.index)
        elif idx == num_blocks-2:
            hidden_weights_df = pd.DataFrame(hidden_weights,index=tf_gene_df.index)
        else:
    ...
    for idx, layer_outs in enumerate(layer_out_arrs):
        if idx == num_blocks-1:
            hidden_df = pd.DataFrame(layer_outs, index=globals()["y_" + phase + "_indices"],columns=tf_gene_df.index)
        else:
            hidden_df = pd.DataFrame(layer_outs, index=globals()["y_" + phase + "_indices"])
        hidden_df.to_csv(dirs_map["hidden_dir"] + '/hidden_outs_' + str(idx) + '.csv')
